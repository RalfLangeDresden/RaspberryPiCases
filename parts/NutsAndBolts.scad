...
//  hex_nut (height, thread_d, size, tolerance, quality, bool_cut, *thread, *pitch);
    ...
    translate ([40,   0, 0]) hex_nut (  8,   3,   8,   0.1, 32, 1, "metric"  /*, 0.425*/);
...
//  washer (outer, inner, width, tolerance, quality, thread);
    ...
    // Konische Innensechskantschraube
        ...
        if ( thread ) { // is set?
            if ( thread == "metric" ) { // is metric?
                if ( bool_cut ) {
                    ...
                    radius = hex_dia (size - tolerance) / 2;
                    cylinder (r = radius, h = height); // hex nut
                }
            }
